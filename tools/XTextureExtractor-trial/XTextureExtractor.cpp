...
#ifdef MUSLIMSIM_LOCAL_TRIAL
#include "muslimsim_sd_stream.h"
#include "muslimsim_flight_stream.h"
#endif
#if LIN || APL
#include <unistd.h>
#endif
	...
	for (int i = start_texture_id; i >= 0; i--) {
		...
		if ((tw == cockpit_texture_width) && (th == cockpit_texture_height) && (tf == cockpit_texture_format)) {
			...
			if ((texture_temp[0] == 0xFF) && (texture_temp[1] == 0x00) && (texture_temp[2] == 0x00) && (texture_temp[3] == 0xFF)) {
				...
				// MuslimSim first-stage trial: local texture/snapshot only.
#ifndef MUSLIMSIM_LOCAL_TRIAL
				if (!network_started) {
...
#ifdef MUSLIMSIM_LOCAL_TRIAL
	// Fixed-size read, no subscriptions/hardware. Unknown or unpowered is black.
	static XPLMDataRef trial_buses = XPLMFindDataRef("AirbusFBW/ACBusVoltages");
	float trial_volts[16] = {};
	int trial_count = trial_buses ? XPLMGetDatavf(trial_buses, trial_volts, 0, 16) : 0;
	bool trial_powered = false;
	for (int i = 0; i < trial_count && i < 16; ++i)
		trial_powered |= trial_volts[i] > 1 && trial_volts[i] < 200;
	ms_sd_publish(trial_powered && cockpit_aircraft_known && !cockpit_dirty && cockpit_texture_draw > 10,
		cockpit_texture_id, cockpit_texture_width, cockpit_texture_height, cockpit_aircraft_filename);
	ms_flight_publish(trial_powered && cockpit_aircraft_known && !cockpit_dirty && cockpit_texture_draw > 10,
		cockpit_texture_id, cockpit_texture_width, cockpit_texture_height, cockpit_aircraft_filename);
	if (!trial_powered) {
		XPLMSetGraphicsState(0, 0, 0, 0, 0, 0, 0);
		glColor3f(0, 0, 0);
		glBegin(GL_QUADS);
		glVertex2i(l, b); glVertex2i(r, b); glVertex2i(r, t); glVertex2i(l, t);
		glEnd();
		return;
	}
#endif
